...
pub enum ExecuteMsg {
    ...
    /// Commits a sequence of transfers to the on-chain state.  If committing any of the transfers
    /// returns an error, then the entire transaction is aborted and none of the transfers are
    /// committed.
    CommitTransfers {
        /// A serialized `Vec<accounting::state::Transfer>`.  Multiple transfers can be committed
        /// together to reduce transaction overhead.
        transfers: Binary,

        /// The index of the guardian set used to sign `transfers`.
        guardian_set_index: u32,

        /// A quorum of signatures for `transfers`.
        signatures: Vec<Signature>,
    },
}
